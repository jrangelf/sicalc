...
def obitoslista(request):
	...
	if request.method == "POST":
		...
		#z = request.FILES['nome_arquivo']
			
		# x é uma string simple e y é uma lista.
		if len(x) > len(y):
		...
		if total_invalidos > 1:
			...
			if total_invalidos == 1:
				mdg = str(total_invalidos) + ' CPF inválido.'
				messages.success (request, (msg)) 
		...
		return render(request, 'obitos-lista.html',{})
...
def tabelasdcp(request):
	...
	if request.method == 'POST':
		lista_registros=[]
		print ("[função: tabelasdcp] ---- via POST ---------")
